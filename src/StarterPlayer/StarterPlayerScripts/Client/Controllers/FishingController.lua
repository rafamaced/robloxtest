...
-- Unlocked zones cache (fetched from PlayerDataService on startup)
local unlockedZones = { "ShallowReef" }
...
--[[
        Determine the player's current zone.
        Reads from cached unlocked zones retrieved from PlayerDataService on startup.
        Returns the first unlocked zone, defaulting to ShallowReef.
    ]]
    function FishingController:GetCurrentZoneId(): string
        if #unlockedZones > 0 then
            return unlockedZones[1]
        end
        return "ShallowReef"
    end
